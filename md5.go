...
func Encrypt(clear_text string) string {
	result = []uint32{A, B, C, D}
	clear_text = hex.EncodeToString([]byte(clear_text))
	clear_text_bin := padding(clear_text)
...
func digest(clear_text_bin string) string {
	...
	for i := 0; i < 4; i++ {
		...
		cipher_text += dec2hex(result[i])
	}
	return cipher_text
}

func div_group(text string) []uint32 {
	X := make([]uint32, 16)
	for i := 0; i < 16; i++ {
		X[i] = bin2dec(text[i*32 : (i+1)*32])
	}
	return X
}
...
func shift(x, n uint32) uint32 {
	return uint32(x<<n) | uint32(x>>((32)-n))
}
